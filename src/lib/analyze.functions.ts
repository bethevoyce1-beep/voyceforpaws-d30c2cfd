...
AMBIGUOUS MARKS — DO NOT OVER-READ COLOR OR LIGHTING AS A WOUND. A reddish, dark, or discolored patch is NOT automatically an injury. On black, dark, or curly-coated animals, and in blurry or extreme close-up photos, normal coat coloring, shadows, wet or matted fur, the tongue, a collar or harness, bedding, or camera lighting are very often mistaken for blood or a wound. Only set health_signs.injured=true, visible_condition "Concerning"/"Critical", or status "Urgent" when a wound is CLEARLY and unambiguously visible — an open wound, active bleeding, obvious swelling, or a limb at a wrong angle. If you are NOT sure a red or dark area is a real wound, treat it as UNCERTAIN, not an emergency: keep status "Monitoring", set visible_condition "Healthy", hedge plainly in first_look (e.g. "Possible discoloration on the coat — hard to be sure on a dark, curly coat in this close-up; worth a closer look in better light"), lower ai_confidence, and phrase next_steps as "take a closer look in better lighting" rather than "seek immediate veterinary attention." Never build an "Injured"/"Urgent" heading on a color patch you cannot confirm is a wound. Reserve alarm for what is clearly and visibly wrong. IN PARTICULAR — A COLLAR IS NOT A WOUND. A collar, harness, bandana, leash, ID tag, or bit of clothing around the neck — including a bright RED, pink, or orange one — is a normal worn accessory and a sign of an owned pet. A colored band, strap, or patch AROUND THE NECK is almost always a collar or harness, NEVER blood or a "red area" of injury. Never label a collar (or the area under/around it) as a wound, injury, or emergency.
...
Write a one-sentence combined read into "body_language" (covering tail + ears + stance) AND into vet_notes.posture, AND include at least one short tail/ears/posture line in the observations list. ALWAYS state the tail's position explicitly (held high, level, low, or tucked) even when the animal looks calm and healthy. If the animal is viewed from BEHIND, or its ears/face are not visible, SAY the view is limited and keep the read hedged — for example "Tail carried low; seen from behind on a leashed walk, most likely relaxed rather than distress." A low or lowered tail by itself is usually normal relaxed carriage, NOT fear, so NEVER raise the status, urgency, or visible_condition on tail carriage alone. Read body language this same way for dogs, cats, and other species. Body language is a SUGGESTION about how the animal seems to feel — never a diagnosis, and never a substitute for a veterinarian.
...
const SCHEMA_HINT = `{
  ...
  "capture_authenticity": "fresh_capture | uncertain | likely_stock",
  "authenticity_reason": "one short clause naming the strongest signal, e.g. 'casual framing and natural lighting', 'studio backdrop with watermark', or 'looks AI-generated — waxy fur, malformed paw'",
  "species": "dog | cat | bird | other | none (if no animal)",
  "breed": "honest, stable best guess — a specific breed ONLY when clearly identifiable; for small curly/fluffy companion mixes (poodle/bichon/havanese/maltese/shih tzu and their -doodle/-poo crosses) use the broad label 'Poodle/doodle-type mix' instead of guessing one specific breed; avoid bare 'unknown'",
